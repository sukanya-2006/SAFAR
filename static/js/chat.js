...
function renderTripPlan(trip) {
  ...
  // ✅ Unique ID per button so onclick always targets the correct trip's data
  const pdfBtnId = "pdf-btn-" + Date.now();
  window.__safar_trips = window.__safar_trips || {};
  window.__safar_trips[pdfBtnId] = trip;

  html += `
    <button class="download-pdf-btn" id="${pdfBtnId}" onclick="generatePDF(window.__safar_trips['${pdfBtnId}'])">
      📄 Download Travel PDF
    </button>
  `;
